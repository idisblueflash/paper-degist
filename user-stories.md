...
4. Given a URL that returns a PDF (happy path)
...
5. Given a URL that returns an HTML paper (web version)
   - when fetch-one classifies the response as `text/html`
     - then save the raw HTML as `files/<name>.html`
     - and defer readable-text extraction to a later stage (US 3)
6. Given a URL that returns a redirect (3xx)
   - when fetch-one follows it (cap the hops)
     - then re-classify the final response
7. Given a paywall / login wall, an error (4xx/5xx/timeout), or any
...
- If the file already exists, skip (so re-runs are idempotent and US 5 can
